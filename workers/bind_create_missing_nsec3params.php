			...
			if (isset($payload['domain'])) {
				...
				if ($domain !== FALSE) {
					...
					if ($generated) {
						...
						$this->getTaskServer()->runBackgroundJob(new JobInfo('', 'bind_zone_changed', ['domain' => $domain->getDomainRaw(), 'change' => 'change'], 'NSEC3 params generated', $this->getCurrentJobId()));
					} else {
                ...
                foreach ($rows as $row) {
                    ...
                    $newjob = new JobInfo('', 'bind_create_missing_nsec3params', ['domain' => $row['domain']], 'Batch NSEC3 param creation', $this->getCurrentJobId());
