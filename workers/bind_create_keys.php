			...
			if (isset($payload['domain'])) {
				...
				if ($domain !== FALSE) {
					...
					if ($generated) {
						...
						$this->getTaskServer()->runBackgroundJob(new JobInfo('', 'bind_zone_changed', ['domain' => $domain->getDomainRaw(), 'change' => 'readd'], 'Applying dnssec-policy after key generation', $this->getCurrentJobId()));
					} else {
